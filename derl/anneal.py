...
class AnnealingVariable(ABC):
    ...
    def step_to(self, val):
        ...
        if val < self.step_count:
            raise ValueError(
                f"val={val} cannot be smaller than self.step_count={self.step_count}"
            )
    ...
    def summarize(self, global_step):
        ...
        summary.add_scalar(
            f"anneal/{self.name}", self.get_tensor(), global_step=global_step
        )
...
class LinearAnneal(AnnealingVariable):
    ...
    def __init__(self, start, nsteps, end=0.0, name=None):
        super().__init__(name)
        self.start = start
        self.nsteps = nsteps
        self.end = end
