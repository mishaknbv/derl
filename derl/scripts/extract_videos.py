...
def video_tags(event_accumulator):
    ...
    return [
        tag
        for tag in event_accumulator.Tags().get("images", [])
...
def extract_videos(logdir, outdir=None, tag=None, fps=15):
    ...
    """
    event_accumulator = EventAccumulator(logdir, size_guidance={"images": 0})
    ...
    for tag_name in tags:
        for image_event in event_accumulator.Images(tag_name):
            ...
            fname = os.path.join(
                outdir, f"{tag_name.replace('/', '_')}_step{image_event.step}.mp4"
            )
...
def main():
    ...
    parser.add_argument(
        "--logdir", required=True, help="path to the directory with events files"
    )
    parser.add_argument(
        "--tag", default=None, help="video summary tag to extract (default: all tags)"
    )
    parser.add_argument(
        "--outdir", default=None, help="output directory (default: logdir/videos)"
    )
    parser.add_argument(
        "--fps",
        type=int,
