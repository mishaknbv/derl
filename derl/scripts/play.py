...
def track(env_id, factory, logdir, args):
  ...
  last = None
  print(f"Tracking {logdir} for new models "
        f"(polling every {args.track_period}s) ...")
  try:
    while True:
      ...
      if is_new_model(model_filepath, last):
        ...
        record(env, policy, output_filepath, seed=args.seed,
               nepisodes=args.nepisodes, fps=args.fps)
...
def get_parser():
  ...
  parser = argparse.ArgumentParser(
      description="Records a video of an agent interacting "
                  "with an environment.")
  ...
  parser.add_argument("--track-period", type=float, default=30.)
  return parser
...
def play(env_id, factory, logdir, args=None):
  ...
  record(env, runner.policy, output_filepath, seed=args.seed,
         nepisodes=args.nepisodes, fps=args.fps)
