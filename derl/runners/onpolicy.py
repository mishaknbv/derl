...
from derl.runners.trajectory_transforms import GAE, MergeTimeBatch, NormalizeAdvantages
        ...
        for interactions in self.runner.run(obs=obs):
            if self.asarray:
                for key, val in filter(
                    lambda kv: kv[0] != "state", interactions.items()
                ):
                    try:
...
class IterateWithMinibatches(RunnerWrapper):
    ...
    def __init__(
        self, runner, num_epochs=3, num_minibatches=4, shuffle_before_epoch=True
    ):
        super().__init__(runner)
