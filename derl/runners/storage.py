...
def recast_numpy(data):
    ...
    return (
        np.array(list(data)).astype(data[0].dtype)
...
class InteractionStorage:
    ...
    def __init__(self, capacity, nstep=3, store_next_observations=False):
        ...
        if store_next_observations and nstep != 1:
            raise ValueError(
                "when store_next_observations is True, "
    ...
    def get(self, indices):
        ...
        if self.store_next_observations:
            result["observations"], result["next_observations"] = map(
                recast_numpy,
                (
                    list(map(itemgetter(0), result["observations"])),
    ...
    def check_next_observations(self, next_observations):
        ...
        if self.store_next_observations and next_observations is None:
            raise ValueError(
                "when store_next_observations was set to True "
    ...
    def add_batch(
        self, observations, actions, rewards, terminations, next_observations=None
    ):
    ...
    def sample(self, size):
        ...
        if self.store_next_observations:
            indices = np.random.randint(
                self.index if not self.is_full else self.capacity, size=size
            )
        ...
        # taken correctly.
        indices = np.random.randint(
            self.index - self.nstep if not self.is_full else self.capacity - self.nstep,
            size=size,
        )
...
class PrioritizedStorage(InteractionStorage):
    ...
    def add(self, observation, action, reward, done, next_observation=None):
        ...
        indices, priorities = [newindex], [0.0]
    ...
    def add_batch(
        ...
        priorities = np.concatenate(
            [np.full(indices.size, self.start_max_priority), np.zeros(newindices.size)],
            0,
        )
    ...
    def sample(self, size):
        ...
        sample["log_probs"] = np.log(self.sum_tree.get_value(indices)) - np.log(
            self.sum_tree.sum
        )
        return sample
