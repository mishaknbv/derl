...
class ExperienceReplay(RunnerWrapper):
    ...
    def initialize_storage(self, obs=None):
        ...
        if self.storage.size != 0:
            raise ValueError(
                f"storage has size {self.storage.size}, but "
    ...
    def __init__(
        ...
        **experience_replay_kwargs,
        ...
        if not hasattr(storage, "update_priorities"):
            raise ValueError("storage does not implement `update_priorities` method")
        if isinstance(beta, (tuple, list)):
            if len(beta) != 2:
                raise ValueError(
                    "beta must be a float, a tuple or a list of length 2 "
                    f"got len(beta)={len(beta)}"
                )
            if runner.nsteps is None:
                raise ValueError(
                    "when beta is a tuple of (start, end) values "
        ...
        super().__init__(runner, storage, anneals=anneals, **experience_replay_kwargs)
        self.alpha = alpha
        self.beta = beta
...
def make_dqn_runner(
    env, policy, num_train_steps, steps_per_sample=4, nlogs=1e5, **wrap_kwargs
):
    """Creates experience replay runner as typically used with DQN alg."""
    runner = EnvRunner(env, policy, horizon=steps_per_sample, nsteps=num_train_steps)
...
class ResampleStorage(RunnerWrapper):
    ...
    def __init__(self, runner, num_samples, batch_size=None):
        if not hasattr(runner, "storage"):
            raise TypeError(
                f"runner must have 'storage' attribute, got runner {runner}"
            )
        if batch_size is None:
            if not hasattr(runner, "batch_size"):
                raise TypeError(
                    "when batch_size is None runner must have 'batch_size' "
...
def sac_runner_wrap(
    ...
    storage_size=1_000_000,
    nstep=1,
    store_next_observations=True,
    batch_size=256,
    **kwargs,
):
    """Wraps runner for SAC algorithm."""
    storage = InteractionStorage(
        storage_size, nstep=nstep, store_next_observations=store_next_observations
    )
    return ExperienceReplay(runner, storage, batch_size=batch_size, **kwargs)


def make_mujoco_sac_runner(
    env,
    policy,
    num_train_steps,
