...
class A2CFactory(Factory):
    ...
    def make_runner(self, env, model=None, nlogs=1e5, **kwargs):
        ...
        runner = EnvRunner(
            env, policy, self.num_runner_steps, nsteps=self.num_train_steps
        )
    ...
    def make_trainer(self, runner, **kwargs):
        self.config |= kwargs
        lr = LinearAnneal(self.lr, self.num_train_steps, 0.0, name="lr")
        optimizer_kwargs = {
            "alpha": self.optimizer_alpha,
            "eps": self.optimizer_epsilon,
        }
        optimizer = RMSprop(
            runner.policy.model.parameters(), lr.get_tensor(), **optimizer_kwargs
        )
        trainer = Trainer(optimizer, anneals=[lr], max_grad_norm=self.max_grad_norm)
        return trainer
