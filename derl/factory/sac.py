...
class SACFactory(Factory):
    ...
    def make_env(self, env_id, **kwargs):
        ...
        return super().make_env(
            env_id, normalize_obs=False, normalize_ret=False, tanh_range_actions=True
        )
    ...
    def make_trainer(self, runner, **kwargs):
        ...
        entropy_scale_opt = Adam(
            (torch.zeros((), requires_grad=True),), self.entropy_scale_lr
        )
        qvalue_opts = [Adam(qv.parameters(), self.qvalue_lr) for qv in model.qvalues]
        trainer = SACTrainer(policy_opt, entropy_scale_opt, qvalue_opts)
        return trainer

    def make_alg(self, runner, trainer, **kwargs):
        self.config |= kwargs
        log_entropy_scale = trainer.entropy_scale_opt.param_groups[0]["params"][0]
        sac = SAC.make(
            runner,
            trainer,
            log_entropy_scale=log_entropy_scale,
