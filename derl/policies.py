        ...
        *distribution_inputs, values = self.model(observations)
        if self.distribution is None:
            if len(distribution_inputs) == 1:
                ...
                raise ValueError(
                    f"model has {len(distribution_inputs)} "
...
class EpsilonGreedyPolicy(Policy):
    ...
    def __init__(self, model, epsilon=0.05, nactions=None, qvalues_from_preds=None):
        self.model = model
    ...
    @classmethod
    def categorical(cls, model, epsilon=0.05, nactions=None, valrange=(-10.0, 10.0)):
