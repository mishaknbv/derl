...
class VideoRecording(Wrapper):
    ...
    def __init__(
        self, env, recording_period, prefix=None, output_filepath=None, max_batch_size=8
    ):
        super().__init__(env)
        ...
        self.frames = []
        bs = min(max_batch_size, getattr(self.env.unwrapped, "nenvs", 1))
    ...
    def step(self, action):
        ...
        if hasattr(self.unwrapped, "nenvs"):
            ...
            max_batch_size = self.had_ended_episodes.shape[0]
            frames = frames[:max_batch_size]
            dones = dones[:max_batch_size]
        else:
        ...
        if (
            np.all(self.had_ended_episodes)
...
class RewardSummarizer:
    ...
    def __init__(self, nenvs, prefix, running_mean_size=100):
        ...
        self.reward_queues = [deque([], maxlen=running_mean_size) for _ in range(nenvs)]
    ...
    def add_summaries(self):
        ...
        summaries[f"reward_mean_{self.qlen}"] = np.mean(
            [np.mean(q) for q in self.reward_queues]
        )

        for key, val in summaries.items():
            summary.add_scalar(f"{self.prefix}/{key}", val, global_step=self.step_count)
...
class Summarize(Wrapper):
    ...
    def reward_summarizer(cls, env, prefix=None, running_mean_size=100):
        ...
        summarizer = RewardSummarizer(
            nenvs, prefix, running_mean_size=running_mean_size
        )
    ...
    def step(self, action):
        ...
        terminations = np.asarray(
            [
                info.get("real_done", done_collection[i])
