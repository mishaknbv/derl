...
import numpy as np
from gymnasium import Env, Space, Wrapper
    ...
    def __init__(self, spaces):
        ...
        for space in spaces:
            if not isinstance(space, first_type):
                raise TypeError(
                    f"spaces have different types: {first_type}, {type(space)}"
                )
            if first_shape != space.shape:
                raise ValueError(
                    f"spaces have different shapes: {first_shape}, {space.shape}"
                )
    ...
    def _check_actions(self, actions):
        if not len(actions) == self.nenvs:
            raise ValueError(
                ...
                f"len(actions) = {len(actions)}, nenvs = {self.nenvs}"
            )
...
def worker(connection, make_env_function, send_spaces=True, **make_env_kwargs):
...
class ParallelEnvBatch(EnvBatch):
    ...
    def __init__(self, make_env, make_env_kwargs, nenvs=None):
        if not isinstance(make_env_kwargs, (tuple, list, np.ndarray)):
            if nenvs is not None:
                raise ValueError(
                    "when make_env_kwargs is not an instance of "
        ...
        self._processes = [
            Process(
                target=worker, args=(connection, make_env), kwargs=kwargs, daemon=True
            )
            for connection, kwargs in zip(self._worker_connections, make_env_kwargs)
        ]
