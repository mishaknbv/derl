...
class StubPolicy:
  ...
  def act(self, obs, state=None, update_state=True):
    del obs, state, update_state
...
def test_reinitialize_init_fn():
  ...
  assert nresets == -1
  assert torch.all(model[0].weight == 0.)
...
def test_intrinsic_reward_wrapper():
  ...
  target = LinearModel()
  predictor.linear.weight.data = torch.tensor([[2.], [3.]])
  predictor.linear.bias.data = torch.tensor([0.5, -1.5])
  target.linear.weight.data = torch.tensor([[1.], [1.]])
  target.linear.bias.data = torch.tensor([0., 0.])
  env = IntrinsicReward(DummyEnv(), predictor, target,
                        normalize=False, intrinsic_gamma=0.99)
  env.reset()
...
def test_rnd_gae():
  ...
  gae = RNDGAE(policy, extrinsic_gamma=0.999, intrinsic_gamma=0.99,
               extrinsic_coef=2., intrinsic_coef=1., normalize=False)
  ...
  expected_advantages = np.array([[[2.473025, 5.13357125]],
                                  [[0.5, 1.2425]]])
  expected_value_targets = np.array([[[2.973025, 7.63357125]],
                                     [[2., 4.7425]]])
  ...
  nt.assert_allclose(trajectory["advantages"],
                     np.array([[10.07962125], [2.2425]]), rtol=0, atol=1e-5)
...
def test_rnd_atari_interactions(rnd_atari_alg):
  ...
  assert_interactions("testdata/rnd/atari/interactions.npz",
                      rnd_atari_alg, rtol=1e-6, atol=1e-6)


def test_rnd_atari_grad(rnd_atari_alg):
  assert_grad("testdata/rnd/atari/grads.pt", rnd_atari_alg,
              rtol=1e-6, atol=1e-6)


def test_rnd_atari_losses(rnd_atari_alg):
  assert_losses("testdata/rnd/atari/losses.pt", rnd_atari_alg,
                rtol=1e-5, atol=1e-5)
