...
class A2CLoss(Loss):
    ...
    def __init__(self, policy, value_loss_coef=0.25, entropy_coef=0.01, name=None):
    ...
    def policy_loss(self, trajectory, act=None):
        ...
        if log_prob.shape != advantages.shape:
            raise ValueError(
                "trajectory has mismatched shapes: "
        ...
        if summary.should_record():
            ...
            for key, val in summaries.items():
                summary.add_scalar(
                    f"{self.name}/{key}", val, global_step=self.call_count
                )
...
class A2C(Alg):
    ...
    def __init__(
        self, runner, trainer, value_loss_coef=0.25, entropy_coef=0.01, name=None
    ):
        loss_fn = A2CLoss(
            runner.policy,
