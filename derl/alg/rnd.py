...
from derl.models import get_device, broadcast_inputs, NatureCNNBase
...
from derl.runners.onpolicy import (
    TransformInteractions, IterateWithMinibatches
)
...
class IntrinsicReward(Normalize):
  ...
  def __init__(self, env, predictor, target,
               normalize=True, clipobs=5.,
               intrinsic_gamma=0.99, eps=1e-8):
    super().__init__(env,
                     obs=normalize,
                     ret=normalize,
                     clipobs=clipobs,
  ...
  def step(self, action):
    ...
    with torch.no_grad():
      ...
      intrinsic_rew = torch.mean(torch.square(
          self.predictor(nobs) - self.target(nobs)), -1).cpu().numpy()
...
class PredictorModel(nn.Module):
  ...
  def __init__(self, base, observation_sample,
               num_extra_layers=2, activation=nn.ReLU):
    super().__init__()
    self.base = base
    with torch.no_grad():
  ...
  def __call__(self, trajectory):
    ...
    trajectory["terminations"] = np.stack(
        [trajectory["terminations"],
         np.zeros_like(trajectory["terminations"])],
        -1
    )
    advantages, value_targets = super().__call__(trajectory)
    trajectory["advantages"] = (self.extrinsic_coef * advantages[..., 0]
...
class RND(PPO):
  ...
  def learn(self, model_dump_period=1.97e9 // 100,
