...
from derl.alg.common import Alg, Loss, Trainer, r_squared
from derl.alg.dqn import TargetUpdater

# pylint: disable=invalid-name
SACLossTuple = namedtuple(
    "SACLossTuple", ["policy_loss", "entropy_scale_loss", "qvalue_losses"]
)
...
class SmoothTargetUpdater(TargetUpdater):
    ...
    def update(self, step_count):
        for tparam, param in zip(self.target.parameters(), self.model.parameters()):
...
class SACLoss(Loss):
    ...
    def policy_loss(self, trajectory, act=None):
        ...
        if log_prob.shape + (1,) != qvalues.shape:
            raise ValueError(
                "elements of act have mismatched shapes: "
        ...
        if summary.should_record():
            summary.add_scalar(
                f"{self.name}/policy_loss", policy_loss, global_step=self.call_count
            )
    ...
    def entropy_scale_loss(self, trajectory, act=None):
        ...
        entropy_scale_loss = -self.log_entropy_scale * (
            log_prob.detach() + target_entropy
        )
        entropy_scale_loss = torch.mean(entropy_scale_loss)

        if summary.should_record():
            summary.add_scalar(
                f"{self.name}/entropy_scale",
                self.entropy_scale,
                global_step=self.call_count,
            )
            summary.add_scalar(
                f"{self.name}/entropy_scale_loss",
                entropy_scale_loss,
                global_step=self.call_count,
            )
    ...
    def compute_targets(self, trajectory, act=None):
        ...
        with torch.no_grad():
            ...
            next_qvalues = torch.min(
                torch.stack(
                    self.target_policy.act(
                        dict(observations=next_obs, actions=next_actions), training=True
                    )["taken_actions_qvalues"]
                ),
                0,
            ).values
            ...
            targets = (
                self.reward_scale * rewards + (1 - terminations) * self.gamma * targets
            )
        return targets
    ...
    def qvalue_losses(self, trajectory, act=None):
        ...
        for i, qpreds in enumerate(qvalues):
            if qtargets.shape != qpreds.shape:
                raise ValueError(
                    "qpreds and qtargets have mismatched shapes, "
        ...
        if summary.should_record():
            summary.add_scalar(
                f"{self.name}/qtargets",
                torch.mean(qtargets),
                global_step=self.call_count,
            )
            for i, loss in enumerate(qvalue_losses):
                summary.add_scalar(
                    f"{self.name}/qpreds_{i}",
                    torch.mean(qvalues[i]),
                    global_step=self.call_count,
                )
                summary.add_scalar(
                    f"{self.name}/r_squared_{i}",
                    r_squared(qtargets, qvalues[i]),
                    global_step=self.call_count,
                )
                summary.add_scalar(
                    f"{self.name}/qvalue_loss_{i}", loss, global_step=self.call_count
                )
...
class SACTrainer(Trainer):
    ...
    def __init__(self, policy_opt, entropy_scale_opt, qvalue_opts, max_grad_norm=None):
    ...
    def optimizer_step(self, optimizer, loss, tag):
        ...
        self.preprocess_gradients(
            chain.from_iterable(group["params"] for group in optimizer.param_groups),
            tag,
        )
        optimizer.step()
