...
def iter_sac_loss_tuple(loss):
    ...
    if not isinstance(loss, SACLossTuple):
        raise TypeError(
            f"loss is expected to be of type SACLossTuple, got type(loss)={type(loss)}"
        )
...
def save_grad(fname, alg):
    ...
    for field, lss in iter_sac_loss_tuple(loss):
        ...
        if set(grads) & set(new_grads):
            raise ValueError(
                f"intersection of gradient keys: {set(grads) & set(new_grads)}"
            )
        grads.update(new_grads)
    torch.save(grads, fname)


def assert_grad(fname, alg, rtol=1e-7, atol=0.0):
    interactions = next(alg.runner.run())
    loss = alg.loss(interactions)
    ...
    for field, lss in iter_sac_loss_tuple(loss):
        ...
        for i, param in enumerate(alg.model.parameters()):
            if param.grad is None:
                ...
                assert_all_close(
                    param.grad, expected[f"{field}/grad_{i}"], rtol=rtol, atol=atol
                )
        alg.model.zero_grad()
...
def assert_losses(filename, alg, rtol=1e-6, atol=0.0):
...
def sac_mujoco_alg():
    ...
    env = make_env("HalfCheetah-v5", seed=0, normalize_obs=False, normalize_ret=False)
...
def test_sac_interactions(sac_mujoco_alg):
    assert_interactions(
        "testdata/sac/mujoco/interactions.npz", sac_mujoco_alg, rtol=1e-5, atol=1e-4
    )


def test_sac_grad(sac_mujoco_alg):
    assert_grad("testdata/sac/mujoco/grads.pt", sac_mujoco_alg, rtol=1e-4, atol=1e-4)


def test_sac_losses(sac_mujoco_alg):
    assert_losses("testdata/sac/mujoco/losses.pt", sac_mujoco_alg, rtol=1e-5, atol=1e-5)
