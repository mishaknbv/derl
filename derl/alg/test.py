...
def assert_interactions(
    fname, alg, ignore_keys=("state", "infos"), rtol=1e-7, atol=0.0
):
    """Checks that interactions have values from the file."""
    ignore_keys = set(ignore_keys) or {}
    interactions = next(alg.runner.run())
    with np.load(fname, allow_pickle=True) as expected:
        assert sorted(interactions.keys()) == sorted(expected.keys())
...
def save_grad(fname, alg):
    ...
    loss.backward()
    grads = {f"grad_{i}": param.grad for i, param in enumerate(alg.model.parameters())}
